pipeline {
  ...
  environment {
    ...
    DOCKERHUB_IMAGE = 'linuxserver/handbrake'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/handbrake'
    PR_DOCKERHUB_IMAGE = 'lspipepr/handbrake'
    DIST_IMAGE = 'arch'
    MULTIARCH = 'false'
    CI = 'true'
    CI_WEB = 'true'
  ...
  stages {
    ...
    stage("Set tag custom bash"){
      steps{
        script{
          env.EXT_RELEASE = sh(
            script: ''' curl -sL https://mirror.rackspace.com/archlinux/extra/os/x86_64/extra.db | tar tzf - | awk -F '(-|/)' '/handbrake/ {print $2; exit}' ''',
            returnStdout: true).trim()
    ...
    stage('Build-Single') {
      ...
      steps {
        ...
        sh "docker buildx build \
          ...
          --label \"org.opencontainers.image.url=https://github.com/linuxserver/docker-handbrake/packages\" \
          --label \"org.opencontainers.image.documentation=https://docs.linuxserver.io/images/docker-handbrake\" \
          --label \"org.opencontainers.image.source=https://github.com/linuxserver/docker-handbrake\" \
          ...
          --label \"org.opencontainers.image.description=[HandBrake](https://handbrake.fr/) is an open-source tool, built by volunteers, for converting video from nearly any format to a selection of modern, widely supported codecs.\" \
